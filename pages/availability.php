...
$pageTitle = t('availability_title');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_availability'])) {
    ...
    if(empty($starts_at) || empty($ends_at)) {
        ...
        if($stmt->execute()) {
            $_SESSION['availability_flash_success'] = t('slot_added');
            header('Location: availability.php');
            exit();
        } else {
            $error = t('slot_add_error');
        }
        $stmt->close();
    }
...
<div class="page-shell page-surface">
    ...
    <div class="layout-sidebar-3">
        ...
        <div class="form-card">
            <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-4"><?php echo t('add_slots_by_time'); ?></h2>
            <form method="POST" class="stack-md">
                <div>
                    <label class="field-label"><?php echo t('start_date'); ?></label>
                ...
                <div>
                    <label class="field-label"><?php echo t('start_time'); ?></label>
                ...
                <div>
                    <label class="field-label"><?php echo t('end_time'); ?></label>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1"><?php echo t('end_date_note'); ?></p>
                </div>

                <div>
                    <label class="field-label"><?php echo t('consultation_type'); ?></label>
                    <select name="consultation_type" class="input-control mb-2">
                        <option value="online" <?php echo $consultation_type === 'online' ? 'selected' : ''; ?>><?php echo t('online'); ?></option>
                        <option value="in_person" <?php echo $consultation_type === 'in_person' ? 'selected' : ''; ?>><?php echo t('in_person'); ?></option>
                    </select>
                    <label class="field-label"><?php echo t('note_optional'); ?></label>
                    <input type="text" name="note" placeholder="<?php echo t('note_placeholder'); ?>" class="input-control" value="<?php echo htmlspecialchars($note); ?>">
                </div>
        ...
        <div class="space-y-4">
            ...
            <?php if(empty($slots)): ?>
                <div class="empty-card">
                    <p class="text-gray-500 dark:text-gray-400"><?php echo t('no_slots_yet'); ?></p>
                </div>
            <?php endif; ?>
